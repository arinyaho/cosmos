...
description: "Convert/sync Codex skills under ~/.codex/skills into Claude configuration under ~/.claude (agents + skills). Use when exporting a Codex skill to teammates who use Claude, or when you need to keep Claude agents/skills updated after editing a Codex skill. Supports dry-run/apply, overwriting, exporting a subset of skills, and name mapping (e.g. pr-reviewer → review)."
---
...
```bash
python3 ~/.codex/skills/codex-to-claude/scripts/sync_codex_to_claude.py \
  --skill pr-reviewer \
...
- Change locations: `--src ~/.codex/skills --dst ~/.claude`
...
- **Claude skill**: `~/.claude/skills/<name>/`
  ...
  - Rewrites common path references from `~/.codex/skills` to `~/.claude/skills` for teammate portability.
