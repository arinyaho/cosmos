...
description: Convert/sync global Claude configuration (`~/.claude/agents` and `~/.claude/skills`) into global Codex skills under `~/.codex/skills` (import agents as skills, add required YAML frontmatter, and copy scripts/resources).
---
...
1) Preview changes (dry-run):
   - `python3 ~/.codex/skills/claude-to-codex/scripts/sync_claude_to_codex.py --dry-run`
2) Apply the sync:
   - `python3 ~/.codex/skills/claude-to-codex/scripts/sync_claude_to_codex.py --apply`
3) Re-run anytime after you edit files under `~/.claude/agents` or `~/.claude/skills`.

## What it converts
- **Claude agents**: `~/.claude/agents/*.md` → `~/.codex/skills/<name>/SKILL.md`
  - Reads the agent frontmatter, keeps only `name`/`description` (Codex format), and preserves the body as the skill instructions.
- **Claude skills**: `~/.claude/skills/<skill>/...` → `~/.codex/skills/<skill>/...`
...
- Writes only under `~/.codex/skills` by default (or `--dst`).
