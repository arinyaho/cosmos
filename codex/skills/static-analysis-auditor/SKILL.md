...
description: "Static analysis workflow for distributed/async pipelines. Use when asked to perform static analysis, find missed bugs, or harden review checklists—especially around error propagation, retry/backoff, long-polling status APIs, storage/network I/O, and job state machines."
---
...
## Scripts
- `~/.codex/skills/static-analysis-auditor/scripts/static_scan.py`: heuristic scan that collects candidate risk points and outputs a markdown report.
  - Example: `python3 ~/.codex/skills/static-analysis-auditor/scripts/static_scan.py --root /path/to/repo --output /tmp/static-scan.md`
- `~/.codex/skills/static-analysis-auditor/scripts/static_triage.py`: summarizes the scan report with top files per section and cross-signal hotspots.
  - Example: `python3 ~/.codex/skills/static-analysis-auditor/scripts/static_triage.py /tmp/static-scan.md --output /tmp/static-triage.md`
